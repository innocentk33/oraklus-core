...
public class TypeLitRestControllerTest {
	...
	public void test() throws Exception {
		...
		mockMvc.perform(post("/relation/typeLit/add")
		...
		mockMvc.perform(delete("/relation/typeLit/delete")
			.header("X-Auth-Token", "X-Token-Bearer " + accessToken)
			.param("id", "1"))
			.andExpect(status().is2xxSuccessful());

		mockMvc.perform(get("/relation/typeLit/find-one")
			.header("X-Auth-Token", "X-Token-Bearer " + accessToken)
			.param("id", "1"))
			.andExpect(status().isOk());

		mockMvc.perform(get("/relation/typeLit/load")
			.header("X-Auth-Token", "X-Token-Bearer " + accessToken))
			.andExpect(status().isOk());
	}
}
