...
public class EtablissementRestControllerTest {
	...
	public void test() throws Exception {
		...
		mockMvc.perform(post("/relation/etablissement/add")
		...
		mockMvc.perform(delete("/relation/etablissement/delete")
			.header("X-Auth-Token", "X-Token-Bearer " + accessToken)
			.param("id", "1"))
			.andExpect(status().is2xxSuccessful());

		mockMvc.perform(get("/relation/etablissement/find-one")
			.header("X-Auth-Token", "X-Token-Bearer " + accessToken)
			.param("id", "1"))
			.andExpect(status().isOk());

		mockMvc.perform(get("/relation/etablissement/load")
			.header("X-Auth-Token", "X-Token-Bearer " + accessToken))
			.andExpect(status().isOk());
	}
}
