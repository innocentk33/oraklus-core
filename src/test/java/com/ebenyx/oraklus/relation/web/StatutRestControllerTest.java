...
public class StatutRestControllerTest {
	...
	public void test() throws Exception {
		...
		mockMvc.perform(post("/relation/statut/add")
		...
		mockMvc.perform(delete("/relation/statut/delete")
			.header("X-Auth-Token", "X-Token-Bearer " + accessToken)
			.param("id", "1"))
			.andExpect(status().is2xxSuccessful());

		mockMvc.perform(get("/relation/statut/find-one")
			.header("X-Auth-Token", "X-Token-Bearer " + accessToken)
			.param("id", "1"))
			.andExpect(status().isOk());

		mockMvc.perform(get("/relation/statut/load")
			.header("X-Auth-Token", "X-Token-Bearer " + accessToken))
			.andExpect(status().isOk());
	}
}
