...
public class ServiceRestControllerTest {
	...
	public void test() throws Exception {
		...
		mockMvc.perform(post("/relation/service/add")
		...
		mockMvc.perform(delete("/relation/service/delete")
			.header("X-Auth-Token", "X-Token-Bearer " + accessToken)
			.param("id", "1"))
			.andExpect(status().is2xxSuccessful());

		mockMvc.perform(get("/relation/service/find-one")
			.header("X-Auth-Token", "X-Token-Bearer " + accessToken)
			.param("id", "1"))
			.andExpect(status().isOk());

		mockMvc.perform(get("/relation/service/load")
			.header("X-Auth-Token", "X-Token-Bearer " + accessToken))
			.andExpect(status().isOk());
	}
}
